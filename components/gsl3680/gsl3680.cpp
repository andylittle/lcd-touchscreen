...
esphome::i2c::ErrorCode GSL3680::init() {
    ...
    // Guition reference driver (esp_lcd_touch_gsl3680) resets and starts the
    // chip a second time after the firmware upload, then polls the RAM marker.
    STOP_ON_I2C_ERROR(err, this->reset());
    STOP_ON_I2C_ERROR(err, this->start());
...
esphome::i2c::ErrorCode GSL3680::read_ram() {
    ...
    auto err = esphome::i2c::ERROR_OK;
    // Reference driver: up to 10 attempts, 50 ms apart, waiting for 5a 5a 5a 5a.
    for (int attempt = 1; attempt <= 10; attempt++) {
        esphome::delay(50);
        STOP_ON_I2C_ERROR(err, this->read_register(0xb0, (uint8_t *)&buf, 4));
        ESP_LOGD(TAG, "Read RAM (attempt %d/10): %x, %x, %x, %x", attempt, buf[0], buf[1], buf[2], buf[3]);
        if (buf[0] == 0x5a && buf[1] == 0x5a && buf[2] == 0x5a && buf[3] == 0x5a) {
            ESP_LOGI(TAG, "GSL3680 firmware running");
            return err;
        }
    }
    ESP_LOGE(TAG, "GSL3680 firmware did not start (RAM marker never became 0x5a)");
    return esphome::i2c::ERROR_UNKNOWN;
}
